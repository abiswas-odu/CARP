...
bool isNOtPrevFound(map<string,string> &allISSeqs, string hitSeq ,IRepeat ir)
{
    for(std::map<string,string>::iterator iter = allISSeqs.begin(); iter != allISSeqs.end(); iter++) {
        int matches=0,mismatches=0,gaps=0,hitLen=0;
		cout<<(*iter).second<<"\n"<<hitSeq.substr(ir.getIrlstart(),(ir.getIrrend()-ir.getIrlstart()+1));
		double score = nwalign((*iter).second,hitSeq.substr(ir.getIrlstart(),(ir.getIrrend()-ir.getIrlstart()+1)),matches,mismatches,gaps,hitLen);
        double minMatchesCtr = (*iter).second.size()*MIN_GLOBAL_MATCHES_THRESH;
        if(matches >= minMatchesCtr)
		{
...
map<string,string> expandTransposaseSeqs(map<string,string> &hitSeqs, map<string,string> &rawReads, map<string,string> &allISSeqs,string readFilePath, string blast_path, int iterCtr, string baseName,string writeIntermideateFiles)
{
	...
	for(std::map<string,string>::iterator iter = hitSeqs.begin(); iter != hitSeqs.end(); iter++) {
		...
		if(uniqBeginMatches.size()>=MIN_CLASS_SIZE && uniqEndMatches.size()>=MIN_CLASS_SIZE && foundIR && isNOtPrevFound(allISSeqs,thisHitSeq,ir))
		{
			...
			fastaFile.close();
			//update final list
			allISSeqs.insert(pair<string,string>(thisHitSeqName,thisHitSeq.substr(ir.getIrlstart(),(ir.getIrrend()-ir.getIrlstart()+1))));
			//write SAM file
		...
		int childSeqID=0;
		if(thisHitSeq.size() <= MAX_SEED_LENGTH && !isFinalISForm)
		{
...
vector<string> findTransposaseHits(ifstream& ctgBlastFile,string keywords,string genbank_path, string wget_path)
{
	...
	for (m_it = blastStrings.begin();  m_it != blastStrings.end();  m_it = s_it)
    {
        ...
        if(std::find(blackListAcc.begin(), blackListAcc.end(), accCode)==blackListAcc.end())   //Check black listed acc codes
        {
            // Load genbank File
...
int main(int argc, char **argv)
{
	...
	string cmdStr = blast_path + "blastn -task megablast -db "+ blast_db_path +"nt -evalue 1e-01 -num_threads 32 -query " + string(argv[2]) + " -out ctgBLASTResult.txt -outfmt \"10 qseqid sacc stitle salltitles qstart qend sstart send\"";
    int ret = system(cmdStr.c_str());
	...
	isRelatedCtgBLASTResult = findTransposaseHits(ctgBlastFile,annotationSearchKeywords,genbank_path,wget_path);
    ...
    map<string,string> allISSeqs;
    do{
		if(writeIntermideateFiles == "YES")
			create_directory("level"+SSTR(loopCtr));
		hitSeqs = expandTransposaseSeqs(hitSeqs,rawReads, allISSeqs, string(argv[3]), blast_path,loopCtr, string(argv[4]),writeIntermideateFiles);
		loopCtr++;
	}while(hitSeqs.size()>0 && loopCtr < maxLoopCtr);

	cout<<"Writing Final Result Fasta File\n";
    string fastaFileName = "FinalResult/" + string(argv[4])+"_allFinalIS.fasta";
	ofstream fastaFile;
	fastaFile.open(fastaFileName.c_str());
	for(std::map<string,string>::iterator iter = allISSeqs.begin(); iter != allISSeqs.end(); iter++) {
        fastaFile<<">"<<(*iter).first<<"\n";
        fastaFile<<(*iter).second<<"\n";
	}
	fastaFile.close();
	readFile.close();
    ctgBlastFile.close();
